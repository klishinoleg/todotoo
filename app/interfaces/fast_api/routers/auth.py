...
async def oauth_callback(
    ...
    try:
        action_type = service.resolve_action_type(provider_type, data.state, data.action_type)
        ...
        if action_type == AuthActionType.REGISTER:
            return await AuthUseCase().register(
                AuthRequestOAuthSignUpDTO(
                    provider_type=provider_type,
        ...
        return await AuthUseCase().login(
            AuthRequestOAuthLoginDTO(
                provider_type=provider_type,
                action_type=action_type,
                provider_data=provider_data_dto,
            )
